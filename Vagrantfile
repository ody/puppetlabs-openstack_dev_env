    ...
    config.vm.define name.intern do |agent|
      ...
      if operatingsystem and operatingsystem != ''
        ...
        when 'redhat'
          os_name          = 'centos'
          agent.vm.box     = 'centos'
          agent.vm.box_url = 'https://dl.dropbox.com/u/5721940/vagrant-boxes/vagrant-centos-6.4-x86_64-vmware_fusion.box'
        when 'ubuntu'
          os_name          = 'precise64'
          agent.vm.box     = 'precise64'
          agent.vm.box_url = 'http://files.vagrantup.com/precise64_vmware_fusion.box'
        else
          raise(Exception, "undefined operatingsystem: #{operatingsystem}")
        end
      end
